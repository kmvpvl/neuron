...
export default class NeuronComponent extends React.Component<INeuron, INeuronState> {
    ...
    render(): React.ReactNode {
        const ndata = Neuron.getSValues(this.neuron);
        const width = 50;
        ...
        let s_l = s.map((v, i)=><>
            <circle cx={circle_diameter/2} cy={v} r={circle_diameter/2} key={`neuron_${this.neuron._name}_s_${i}`} stroke='silver' fill='white'></circle>
            <circle cx={circle_diameter/2} cy={v} r={circle_diameter/2 * ndata[i]} key={`neuron_${this.neuron._name}_s_${i}`} ></circle>
        </>)
